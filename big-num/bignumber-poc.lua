...
function Big:log10()
	...
	if self.e == nan and self.m == nan then
		return 0
	end
	if self:lte(Big:new(0)) then
		return 0
	end
...
function Big:compare(b)
	...
	if self.e > b.e and self:is_positive() then
		return 1
	end
	if self.e < b.e and self:is_positive() then
		return -1
	end
	if self.e > b.e and self:is_negative() then
		return -1
	end
	if self.e < b.e and self:is_negative() then
		return 1
...
function format123(value, places)
	places = places or 3
